...
def legacy_db(_unused=None):
    """Tables as the very first version of the app left them, in the test schema.

    The upgrade path still matters: an old backup restored into Postgres has
    the same missing columns and unmatched names as an old SQLite file did.
    """
    import db
    with db.get_conn() as conn:
        conn.execute("""
            CREATE TABLE students (id INTEGER GENERATED BY DEFAULT AS IDENTITY
                PRIMARY KEY, name TEXT NOT NULL, gender TEXT, privileges TEXT)""")
        conn.execute("""
            CREATE TABLE schedules (id INTEGER GENERATED BY DEFAULT AS IDENTITY
                PRIMARY KEY, meeting_date TEXT, meeting_type TEXT, part_name TEXT,
