...
def resync_identities():
    ...
    with get_conn() as conn:
        for table in ("students", "schedules", "audit_log", "snapshots",
                     "no_meeting_periods", "outgoing_engagements"):
            conn.execute(
...
def init_db():
    with get_conn() as conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS no_meeting_periods (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS outgoing_speakers (
                student_id INTEGER PRIMARY KEY,
                talk_numbers TEXT,
                active INTEGER DEFAULT 1
            )""")
        conn.execute("""
            CREATE TABLE IF NOT EXISTS outgoing_engagements (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                student_id INTEGER NOT NULL,
                meeting_date TEXT NOT NULL,
                congregation TEXT,
...
# =============================================================================
# NO-MEETING WEEKS (assemblies, conventions)
# =============================================================================
@st.cache_data(show_spinner=False)
...
def _forget_no_meeting():
    _no_meeting_periods.clear()
...
# =============================================================================
@st.cache_data(show_spinner=False)
def _outgoing_speakers(schema_name):
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT student_id, talk_numbers FROM outgoing_speakers "
            "WHERE active = 1").fetchall()
    return {sid: [n.strip() for n in (talks or "").split(",") if n.strip()]
            for sid, talks in rows}


def _forget_outgoing_speakers():
    _outgoing_speakers.clear()


def get_outgoing_speakers():
    """{student_id: [talk numbers prepared]} for everyone approved to speak
    at other congregations."""
    return _outgoing_speakers(schema())


def set_outgoing_speaker(student_id, talk_numbers):
    """Approve (or update) this person as an outgoing speaker."""
    numbers = ", ".join(dict.fromkeys(nfc(str(n)) for n in talk_numbers if nfc(str(n))))
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO outgoing_speakers (student_id, talk_numbers, active) "
            "VALUES (?, ?, 1) ON CONFLICT(student_id) DO UPDATE SET "
            "talk_numbers = excluded.talk_numbers, active = 1",
            (student_id, numbers))
    _forget_outgoing_speakers()
    log_change("Outgoing speaker approved", f"id {student_id}: {numbers}")


def remove_outgoing_speaker(student_id):
    with get_conn() as conn:
        conn.execute("UPDATE outgoing_speakers SET active = 0 WHERE student_id = ?",
                    (student_id,))
    _forget_outgoing_speakers()
    log_change("Outgoing speaker removed", f"id {student_id}")


def add_outgoing_engagement(student_id, meeting_date, congregation="", talk_number=""):
    """Record a date this person speaks at another congregation, so the local
    schedule leaves them out that day — the same way an away date does."""
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO outgoing_engagements (student_id, meeting_date, "
            "congregation, talk_number) VALUES (?, ?, ?, ?) "
            "ON CONFLICT(student_id, meeting_date) DO UPDATE SET "
            "congregation = excluded.congregation, talk_number = excluded.talk_number",
            (student_id, str(meeting_date), nfc(congregation), nfc(str(talk_number))))
    _forget_outgoing_engagements()
    log_change("Outgoing engagement added", f"id {student_id}: {meeting_date}")


def delete_outgoing_engagement(engagement_id):
    with get_conn() as conn:
        conn.execute("DELETE FROM outgoing_engagements WHERE id = ?", (engagement_id,))
    _forget_outgoing_engagements()
    log_change("Outgoing engagement removed", f"id {engagement_id}")


def outgoing_engagements(student_id):
    """[(id, date, congregation, talk_number), ...] for this speaker."""
    with get_conn() as conn:
        return conn.execute(
            "SELECT id, meeting_date, congregation, talk_number "
            "FROM outgoing_engagements WHERE student_id = ? ORDER BY meeting_date",
            (student_id,)).fetchall()


@st.cache_data(show_spinner=False)
def _outgoing_on_dates(schema_name):
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT student_id, meeting_date FROM outgoing_engagements").fetchall()
    out = {}
    for sid, d in rows:
...
def _forget_outgoing_engagements():
    _outgoing_on_dates.clear()


def get_outgoing_on(meeting_date):
    """student_ids speaking at another congregation on this date."""
    return _outgoing_on_dates(schema()).get(str(meeting_date), set())
