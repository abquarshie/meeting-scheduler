...
from auth import current_user_role, ROLE_ADMIN, ROLE_COORDINATOR

# Full navigation list mapped with allowed roles ("admin" or "all")
NAV = [
    ...
    ("Manage Participants", "nav_participants", ":material/group:", ROLE_ADMIN),
    ("Upload PDF Brochure", "nav_workbook", ":material/menu_book:", ROLE_ADMIN),
    ("Reports", "nav_reports", ":material/bar_chart:", ROLE_ADMIN),
    ("Admin", "nav_admin", ":material/admin_panel_settings:", ROLE_ADMIN),
]
...
def sidebar(current):
    ...
    with st.sidebar:
        ...
        for page, key, icon, permission in NAV:
            # If the user is Talk Coordinator, filter out pages restricted to Admin
            if role == ROLE_COORDINATOR and permission == ROLE_ADMIN:
                continue
