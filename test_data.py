...
def test_recency_is_measured_from_the_meeting_not_today(core):
    ...
    meeting = "2026-10-14"
    # one threshold: on in the last week means resting, otherwise available.
    # The words still carry the exact distance.
...
def test_saving_survives_odd_values_in_the_slots(core, people):
    """Slot values arrive via a workbook parse, a JSON round trip, a data
    editor and a set of widgets. Postgres refuses a string where an integer
    belongs, so the save coerces rather than trusting all of them."""
    import math

    slots = core.build_midweek_slots(core.default_midweek_parts())
    names = dict(zip(core.get_students()["id"], core.get_students()["name"]))

    # the shapes those paths can produce
    slots[1] = {**slots[1], "part_no": "", "minutes": ""}
    slots[2] = {**slots[2], "part_no": "3", "minutes": 4.0}
    slots[3] = {**slots[3], "part_no": float("nan"), "minutes": None}
    slots[4] = {**slots[4], "part_no": None, "minutes": "x"}

    core.save_schedule("2026-09-16", core.MIDWEEK, slots,
                       {0: (people["Kofi Mensah"], None)}, {}, names)
    rows = core.get_schedules()
    week = rows[rows["meeting_date"] == "2026-09-16"]
    assert len(week) == len(slots)

    stored = week.sort_values("sort_order")["part_no"].tolist()
    assert stored[1] is None or math.isnan(stored[1])       # "" became nothing
    assert stored[2] == 3                                   # "3" became a number
    assert stored[4] is None or math.isnan(stored[4])

    # and a person id arriving as a string still saves
    core.save_schedule("2026-09-23", core.MIDWEEK, slots,
                       {0: (str(people["Kofi Mensah"]), None)}, {}, names)
    later = core.get_schedules()
    saved = later[(later["meeting_date"] == "2026-09-23")
                  & (later["sort_order"] == 0)]
    assert int(saved["student_id"].iloc[0]) == people["Kofi Mensah"]


def test_as_int(core):
    assert core.as_int("") is None
    assert core.as_int("x") is None
    assert core.as_int(None) is None
    assert core.as_int(float("nan")) is None
    assert core.as_int("7") == 7
    assert core.as_int(7.0) == 7
    assert core.as_int(True) == 1
