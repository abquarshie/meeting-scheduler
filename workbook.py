...
def render(students_df, t, selected_lang, aux_default):
    ...
    if uploaded_pdf is not None:
        ...
        if not weeks:
            ...
            # Compute the dated copy now, but do not save yet — an upload alone
            ...
            st.caption("Preview — not saved yet.")
            ...
            st.dataframe(pd.DataFrame([
                ...
                for label, w in dated.items()
            ]), width="stretch", hide_index=True)
            if not sure:
                st.warning("The week dates were guessed. Check the first week's "
                           "date after saving, and correct it under **Week dates**.")

            if st.button("Save this workbook", type="primary",
                         icon=":material/save:"):
                ...
                st.success(f"Saved {len(dated)} week(s).")
                st.rerun()
