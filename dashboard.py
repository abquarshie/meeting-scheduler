...
def render(students_df, t, selected_lang, aux_default):
    if current_role() == ROLE_TALKS:
        _render_weekend_only(students_df)
        return
    _render_full(students_df, t, selected_lang, aux_default)


def _render_full(students_df, t, selected_lang, aux_default):
    today = date.today()
...
def _render_weekend_only(students_df):
    """The Talk Coordinator's dashboard: the weekend meeting only. Nothing
    here about the midweek meeting, the workbook, or setup — those are the
    Life and Ministry Overseer's to handle."""
    today = date.today()
    page_header(greeting(), f"{today:%A} {today.day} {today:%B %Y}")

    if students_df.empty:
        st.info("No participants yet. Ask the Life and Ministry Overseer to add them.")
        return

    schedules_df = get_schedules()
    upcoming = sorted(p for p in saved_meetings(schedules_df)
                      if p[1] == WEEKEND and p[0] >= today.isoformat())

    # ---- the next weekend meeting ----------------------------------------------
    with st.container(border=True):
        if upcoming:
            md, mt = upcoming[0]
            rows = schedules_df[(schedules_df["meeting_date"] == md)
            ...
            filled, needed = fill_counts(rows)
            open_n = needed - filled
            detail = talk_text(meta)
            sub = f"Weekend meeting, {days_away(md)}" + (f". {detail}" if detail else "")
            left, right = st.columns([3, 1], gap="large")
            with left:
                st.markdown(f'<div class="ms-next-when">{long_date(md)}</div>'
                            f'<div class="ms-next-meta">{html_escape(sub)}</div>',
            ...
            with right:
                ...
                if st.button("Fill open slots" if open_n else "Edit schedule",
                             icon=":material/edit:",
                             type="primary" if open_n else "secondary",
                             width="stretch", key="home_fill_wk",
                             help="Suggest fills only the empty ones; anyone "
                                  "already chosen stays." if open_n else None):
                    go("Schedule", schedule_mode="Edit saved", edit_meeting=(md, mt))
                if st.button("Print schedule", icon=":material/print:", width="stretch",
                             key="home_print_wk"):
                    go("View Schedules", view_meeting=(md, mt))
        else:
            st.markdown('<div class="ms-next-when">No weekend meeting scheduled yet</div>'
                        '<div class="ms-next-meta">Create the next weekend meeting to '
                        'get started.</div>', unsafe_allow_html=True)
            if st.button("Create a weekend meeting", icon=":material/add:", type="primary"):
                go("Schedule", schedule_mode="Create new",
                   new_meeting_type=WEEKEND, new_meeting_date=today)

    # ---- at a glance -------------------------------------------------------------
    soon = [p for p in upcoming if p[0] <= (today + timedelta(days=28)).isoformat()]
    open_soon = 0
    for md, mt in soon:
        f_, n_ = fill_counts(schedules_df[(schedules_df["meeting_date"] == md)
                                          & (schedules_df["meeting_type"] == mt)])
    ...
    m1, m2 = st.columns(2)
    m1.metric("Open slots in the next 4 weeks", open_soon, border=True)
    m2.metric("Away or suspended this week", len(away), border=True)
    ...
    if later:
        ...
        event = st.dataframe(
            df.drop(columns="_key"), hide_index=True, width="stretch",
            on_select="rerun", selection_mode="single-row", key="home_upcoming_wk",
            column_config={
                "Filled": st.column_config.ProgressColumn(
                    "Filled", min_value=0, max_value=100, format="%d%%"),
                "Open": st.column_config.NumberColumn("Open", width="small"),
